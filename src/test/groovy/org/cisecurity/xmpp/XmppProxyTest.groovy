...
//			collector.sendCollectionResponse(orchestrator.xmppClient.connectedResource, ovalSystemCharacteristics)
	...
	def "Collect system characteristics that have filters"() {
		given: "A blob of XML"
			def xmltext = """
<oval_objects collection-id="oval:org.cisecurity:collection:9999" 
...
"""
		...
		then: "system characteristics are returned"
			assert osc.systemData.collectedItem.size() == 2
			assert osc.systemData.collectedItem[1] instanceof JAXBElement<EnvironmentvariableItem>
			assert ((EnvironmentvariableItem)osc.systemData.collectedItem[1].value).name.value == "COMPUTERNAME"
	}

	def "Collect system characteristics that has a set"() {
		given: "A blob of XML"
			def xmltext = """
<oval_objects collection-id="oval:org.cisecurity:collection:9999" 
    xmlns="http://oval.cisecurity.org/XMLSchema/oval-collections-6"
    ...
    <objects>
        ...
        <ind-coll:environmentvariable_object id="oval:org.cisecurity:obj:3" version="1" comment="The 'env' environment variable">
            <ind-coll:name>env</ind-coll:name>
        </ind-coll:environmentvariable_object>
        
        <ind-coll:environmentvariable_object id="oval:org.cisecurity:obj:4" version="1" comment="The COMPUTERNAME and env environment variables, using a set">
            <oval-coll:set set_operator="UNION">
                <oval-coll:object_reference>oval:org.cisecurity:obj:2</oval-coll:object_reference>
                <oval-coll:object_reference>oval:org.cisecurity:obj:3</oval-coll:object_reference>
            </oval-coll:set>
        </ind-coll:environmentvariable_object>
    </objects>
</oval_objects>
...
"""
		...
		when: "the orchestrator requests collector's to collect something"
			...
			def osc = orchestrator.performCollection(u, ovalObjects)
		then: "system characteristics are returned"
			assert osc.systemData.collectedItem.size() == 3
			assert osc.systemData.collectedItem[1] instanceof JAXBElement<EnvironmentvariableItem>
			assert ((EnvironmentvariableItem)osc.systemData.collectedItem[1].value).name.value == "COMPUTERNAME"
			assert ((EnvironmentvariableItem)osc.systemData.collectedItem[2].value).name.value == "env"
			assert osc.collectedObjects.collectedObject.find { o -> o.id == "oval:org.cisecurity:obj:4"}.reference.size() == 2
	}
}
